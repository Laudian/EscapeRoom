...
class EscapeRoom(object):
    def __init__(self):
        ...
        self.__players: Dict[Player, discord.user] = {}
        self.__discordUsers: Dict[discord.user, Player] = {}

        # Dictionaries to translate between Rooms and Discord channels
        self.__discordChannels: Dict[Room, discord.channel] = {}
        self.__rooms: Dict[discord.channel, Room] = {}
    ...
    def handle_command(self, caller: discord.user, command, content: str):
        logging.debug("1")
    ...
    # @target is
    def send_message(self, target: Player, content):
    ...
    def discord_to_player(self, user: discord.user) -> Player:
        return self.__discordUsers.get(user)

    # translates a Player user into a Discord User
    # returns None if that user is not in the game
    def player_to_discord(self, player: Player) -> discord.user:
    ...
    def register_player(self, user: discord.user):
        if user in self.get_discord_users():
            ...
            self.send_message(player, "Du wurdest erfolgreich angemeldet")
            logging.debug("User {name} was successfully registered".format(name=player.name))
            self.room.enter(player)
            return

    # translates a discord channel into a game Room
    # returns None if that channel is not in the game
    def discord_to_room(self, channel: discord.channel) -> Room:
    ...
    def room_to_discord(self, room: Room) -> discord.channel:
    ...
    def register_rooms(self):
        channel = self.bot.get_channel(784596953496813598)
        self.__discordChannels[channel] = self.room
        self.__rooms[self.room] = channel
